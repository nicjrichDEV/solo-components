  ...
  const navLinks = componentNames.map((comp, index) => {
    return (
      <a
        href={`#${comp}`}
        key={index}
        className="flex h-8 min-h-8 items-center justify-items-start border-b border-b-neutral-200 px-2 text-sm font-medium text-neutral-600 transition-colors duration-150 ease-in hover:bg-neutral-200"
      >
